...
本站照片皆下載自 Wikimedia Commons 後自架（不 hotlink），統一縮圖並轉為 WebP 格式。
所有圖片的修改內容：縮小尺寸、轉檔 WebP、依版面比例裁切顯示（CSS `object-fit: cover`）。

CC BY / CC BY-SA 圖片依授權條款標示作者、來源與授權；CC BY-SA 圖片之衍生檔案（本 repo 內的 WebP 檔）同樣以相同授權釋出。

## 站台共用

| 檔案 | 內容 | 作者 | 授權 | 來源 |
|---|---|---|---|---|
| `public/images/hub-hero.webp` | 河津川河津櫻與踊り子號列車 | 9689tatsuya | [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) | [Wikimedia Commons](https://commons.wikimedia.org/wiki/File:Shizuoka-Kawazu_cherry_blossoms_and_Odoriko-xl.jpg) |
...
2. 下載原檔後轉為 WebP：封面寬 1600px、每日照片寬 1200px，品質以檔案 < 400KB 為原則。
3. 放入 `trips/{id}/photos/`（行程照片）或 `public/images/`（站台共用）。
4. 在本檔對應表格新增一列：檔案、內容、作者、授權（附條款連結）、來源（附原始頁面連結）。
5. 在 `itinerary.json` 的 `meta.cover.credit` 或 `days[].photo.credit` 填入「作者 / 授權」字串，前端會顯示於圖片角落。
